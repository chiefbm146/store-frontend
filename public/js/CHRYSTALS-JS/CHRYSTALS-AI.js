...
function configureFromScriptTag() {
    STORE_ID = 'chrystal-sparrow'; // Chrystal Sparrow - Artist & Healer
    BACKEND_URL = 'https://stores-backend-phhl2xgwwa-uc.a.run.app';
    ...
    console.log(`[AI Chat] ✅ Configured for store: ${STORE_ID}`);
    console.log(`[AI Chat]    Backend: ${BACKEND_URL}`);
    console.log(`[AI Chat]    Endpoints: ${CHAT_ENDPOINT}, ${TTS_ENDPOINT}`);
    console.log(`[AI Chat]    Voice: en-US-Studio-O (Female)`);
    return true;
}
...
function injectFontAwesome() {
    if (document.getElementById('aiChatFontAwesome')) return; // Already injected
    ...
    console.log('[AI Chat] Font Awesome CDN injected');
}

// ========================================
// DETECT PAGE AND GET THEME COLORS
// ========================================
function getThemeColors() {
    const path = window.location.pathname;

    // Check if we're on Chrystal's page
    if (path === '/chrystal' || path.includes('chrystals store') || path.includes('CHRYSTAL')) {
        return {
            primary: '#A0522D',
            secondary: '#FF8A7B',
            primaryRgb: '160, 82, 45',
            secondaryRgb: '255, 138, 123',
            name: 'chrystal-rust'
        };
    }

    // Check if we're on booking page
    if (path === '/booking' || path.includes('store-booking')) {
        return {
            primary: '#f97316',
            secondary: '#fb923c',
    ...
    // Default to cyan/teal for home and other pages
    return {
        primary: '#0891b2',
        secondary: '#06b6d4',
...
function injectChatCSS() {
    ...
    style.textContent = `
        ...
        .ai-chat-icon {
            ...
            cursor: pointer;
            z-index: 9998;
            box-shadow: 0 4px 20px rgba(${theme.primaryRgb}, 0.4);
            transition: all 0.3s ease;
            border: none;
        }
        .ai-chat-icon:hover {
            transform: scale(1.15);
            box-shadow: 0 8px 30px rgba(${theme.primaryRgb}, 0.6);
        }
        ...
        .chat-modal-header {
            ...
            box-shadow: 0 4px 20px rgba(${theme.primaryRgb}, 0.3);
        }
        .chat-modal-title {
            font-size: 1.5rem;
            font-weight: 700;
        ...
        .chat-modal-input {
            ...
            gap: 12px;
            padding: 20px;
            background: linear-gradient(135deg, rgba(${theme.primaryRgb}, 0.1) 0%, rgba(${theme.secondaryRgb}, 0.1) 100%);
            border-top: 2px solid rgba(${theme.primaryRgb}, 0.2);
        }
        ...
        .chat-modal-input input:focus {
            ...
            box-shadow: 0 0 0 3px rgba(${theme.primaryRgb}, 0.1);
        }
        .chat-modal-input button {
            ...
            cursor: pointer;
            transition: all 0.3s ease;
            box-shadow: 0 4px 15px rgba(${theme.primaryRgb}, 0.3);
        }
        .chat-modal-input button:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(${theme.primaryRgb}, 0.4);
        }
        ...
        .message-play-btn {
            ...
            height: 36px;
            border-radius: 50%;
            border: 2px solid rgba(${theme.primaryRgb}, 0.3);
        ...
        .mobile-chat-message.ai {
            ...
            align-self: flex-start;
            color: #1a1a1a;
            border: 1px solid rgba(${theme.primaryRgb}, 0.1);
        ...
        /* === RESPONSIVE === */
        /* Desktop: Show desktop chat icon, hide mobile FAB */
        ...
        /* Mobile: Hide desktop chat icon, show mobile FAB */
    ...
    console.log('[AI Chat] CSS styles injected');
}
...
function injectChatHTML() {
                    ...
                    <div class="messages" id="aiMessages">
                        <div class="message-wrapper ai" id="desktop-greeting-wrapper">
                            <div class="message ai-message" id="desktop-initial-greeting">
                                Welcome! I'm Chrystal's AI assistant. I can help you learn about her art, cultural work, and services. Ask me anything!
                            </div>
                            <button class="message-play-btn" id="desktop-greeting-tts" data-text="Welcome! I'm Chrystal's AI assistant. I can help you learn about her art, cultural work, and services. Ask me anything!">
                                <i class="fa-solid fa-volume-xmark"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div class="mobile-chat-container">
                    <div class="mobile-chat-messages" id="mobileChatMessages">
                        <div class="message-wrapper ai" id="mobile-greeting-wrapper">
                            ...
                            <button class="message-play-btn" id="mobile-greeting-tts" data-text="Welcome! I'm Chrystal's AI assistant. I can help you learn about her art, cultural work, and services. Ask me anything!">
                                <i class="fa-solid fa-volume-xmark"></i>
                            </button>
                        </div>
                    </div>
    ...
    console.log('[AI Chat] HTML elements injected');
}
...
function initializeChatSystem() {
    // *** CRITICAL: Configure from script tag FIRST ***
    const configured = configureFromScriptTag();
    if (!configured) {
        console.error('[AI Chat] ❌ Failed to configure from script tag. Initialization aborted.');
        return;
    }

    console.log('[AI Chat] Initializing system...');
    ...
    // Note: Chat icons visibility is controlled by CSS media queries, not JavaScript
    // This ensures proper responsive behavior without inline style overrides

    console.log('[AI Chat] ✅ System initialized - Universal Widget Ready');
}

function initializeChatSession() {
    CHAT_SESSION_ID = 'session_' + crypto.randomUUID();
    console.log('[AI Chat] New desktop session:', CHAT_SESSION_ID);
}

function initMobileChatSession() {
    MOBILE_CHAT_SESSION_ID = 'mobile_' + crypto.randomUUID();
    console.log('[AI Chat] New mobile session:', MOBILE_CHAT_SESSION_ID);
}
        ...
        if (enableBtn) {
            enableBtn.onclick = async () => {
                try {
                    ...
                    console.error('[TTS] ❌ Unlock failed:', error);
                    hideAudioModal();
                    resolve(false);
                }
            };
...
async function getTTSAudio(text) {
    ...
    try {
        const response = await fetch(`${BACKEND_URL}${TTS_ENDPOINT}`, {
            ...
            body: JSON.stringify({
                text: text,
                voice: 'en-US-Studio-O'  // Forces Female Voice for Chrystal
            })
        });
...
function attachSpecialTagListeners(container, inputElement, sendFunction) {
    container.querySelectorAll('special').forEach(el => {
        ...
        el.onclick = (e) => {
            ...
            console.log('[Chat] Clicked special tag:', text);
...
function openAIChat() {
    ...
    if (container && window.Water4Chat && !waterInstance) {
        ...
        console.log('[AI Chat] Water background initialized');
    }
}
...
function closeAIChat() {
    ...
    if (waterInstance) {
        ...
        waterInstance = null;
        console.log('[AI Chat] Water background destroyed');
    }
}
...
async function sendMessage(isMobile = false) {
    ...
    // *** CLEAR EVERYTHING - FRESH START EVERY MESSAGE ***
    ...
    try {
        ...
        console.log('[AI Chat] Sending:', userText.substring(0, 50) + '...');
        ...
        console.log('[AI Chat] Response:', aiResponse.substring(0, 50) + '...');
...
function openMobileChat() {
    ...
    if (container && window.Water4Chat && !waterInstance) {
        waterInstance = Object.assign({}, window.Water4Chat);
        waterInstance.init(container);
        console.log('[AI Chat] Water background initialized (mobile)');
    }
}
...
function closeMobileChat() {
    ...
    if (waterInstance) {
        waterInstance.destroy();
        waterInstance = null;
        console.log('[AI Chat] Water background destroyed (mobile)');
    }
}
...
function setupEventListeners() {
    ...
    console.log('[AI Chat] Event listeners setup complete');
}
...
window.AIChat = {
    ...
    // Utility methods
    setStoreId: (id) => {
        console.warn('[AI Chat] Store ID should be set via <script data-store-id="...">. Setting via setStoreId is not recommended.');
    },
    getStoreId: () => STORE_ID,
    getBackendUrl: () => BACKEND_URL
};

console.log('[AI Chat] ✅ Universal Widget v3.0 Loaded');
console.log('[AI Chat] Store: ' + (STORE_ID || 'NOT CONFIGURED'));
console.log('[AI Chat] Use window.AIChat to control chat.');
