...
function initializeChatSystem() {
    ...
    // *** NEW: Proactive Backend Wakeup Ping ***
    (async () => {
        try {
            // Construct the store-specific wakeup URL
            const wakeupUrl = `${BACKEND_URL}/api/v1/${STORE_ID}/wakeup`;
            // Send the request but don't wait for it to complete ('fire-and-forget')
            fetch(wakeupUrl).catch(() => {});
        } catch (e) {
            // Silent error handling
        }
    })(); // Self-invoking async function to not block initialization
...
window.AIChat = {
    ...
    // Utility methods
    setStoreId: (id) => {
        // Store ID should be set via <script data-store-id="...">
    },
    getStoreId: () => STORE_ID,
