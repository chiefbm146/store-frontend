...
def is_bot_request(request_obj) -> bool:
    ...
    EXCEPTION: Allows Stripe webhook requests (they have Stripe-Signature header).
    """
    # EXCEPTION: Allow Stripe webhooks - they have Stripe-Signature header
    if request_obj.headers.get('Stripe-Signature'):
        return False  # Not a bot, it's Stripe
    ...
    # EXCEPTION: Allow Stripe User-Agent
...
logger.critical(f"🔑 STRIPE_WEBHOOK_SECRET LOADED: {'YES' if STRIPE_WEBHOOK_SECRET else 'NO'}")
...
GMAIL_APP_PASSWORD = os.getenv("GMAIL_APP_PASSWORD")  # Gmail App Password (NOT your regular password, no spaces)
NOTIFICATION_EMAIL = "korypetersbm146@gmail.com"  # Where to send sale notifications
logger.critical(f"📧 EMAIL NOTIFICATIONS: {'ENABLED' if GMAIL_USER and GMAIL_APP_PASSWORD else 'DISABLED'}")

def send_sale_notification(payment_intent_id, amount, currency, product_name, customer_email=None):
    """Send email notification when a sale is made"""
    if not GMAIL_USER or not GMAIL_APP_PASSWORD:
        logger.warning("[Email] Gmail credentials not configured, skipping notification")
        return False

    try:
        # Create message
        msg = MIMEMultipart('alternative')
        msg['Subject'] = f"💰 New Sale! ${amount/100:.2f} {currency.upper()}"
        msg['From'] = GMAIL_USER
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            <h2 style="color: #22c55e;">💰 New Sale!</h2>
            <table style="border-collapse: collapse; width: 100%; max-width: 400px;">
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>Amount:</strong></td>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;">${amount/100:.2f} {currency.upper()}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>Product:</strong></td>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;">{product_name or 'N/A'}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>Customer:</strong></td>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;">{customer_email or 'N/A'}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>Payment ID:</strong></td>
                    <td style="padding: 10px; border-bottom: 1px solid #eee; font-size: 12px;">{payment_intent_id}</td>
                </tr>
            </table>
            <p style="margin-top: 20px; color: #666;">
                <a href="https://dashboard.stripe.com/payments/{payment_intent_id}" style="color: #6366f1;">View in Stripe Dashboard →</a>
            </p>
        </body>
        </html>
        """
        ...
        logger.info(f"[Email] ✅ Sale notification sent to {NOTIFICATION_EMAIL}")
        return True
...
# =============================================================================
# PRODUCT PRICING (Server-side source of truth - NEVER trust frontend prices)
# =============================================================================
PRODUCT_PRICING = {
    'custom-store': {
        'name': 'Custom Website - One-Time Payment',
        'price': 49900,  # $499.00 in cents
        'description': 'Complete custom website built in 24-48 hours'
    },
    # 'test-buy': {
    #     'name': 'Test Buy',
...
def create_payment_intent():
    ...
    try:
        ...
        # STEP 0: Handle direct product_id (server-side price lookup - SECURE)
        product_id = data.get('product_id')
        if product_id:
            # SECURITY: Look up price from server-side PRODUCT_PRICING, NEVER trust frontend
            product_data = PRODUCT_PRICING.get(product_id)
            if not product_data:
                logger.warning(f"[Payment Intent] Unknown product_id: {product_id}")
                return jsonify(error={'message': f'Unknown product: {product_id}'}), 400

            total_amount_cents = product_data['price']
            metadata['product_id'] = product_id
            metadata['description'] = product_data['description']
            metadata['product_name'] = product_data['name']
            logger.info(f"[Payment Intent] Product purchase: {product_id}, server-side price: ${total_amount_cents/100:.2f}")
        else:
        ...
        # STEP 4: Create Stripe Payment Intent (Direct Payment to YOUR account)
        try:
            # Direct payment - no Connect, no Express accounts
            # Money goes directly to your Stripe account (the one tied to STRIPE_SECRET_KEY)
            intent_params = {
            ...
            # Add receipt_email if provided (for Stripe to send customer receipt)
            customer_email = data.get('customer_email')
            if customer_email:
                intent_params['receipt_email'] = customer_email
                logger.info(f"[Payment Intent] Receipt will be sent to: {customer_email}")

            logger.info(f"[Payment Intent] Creating direct payment: Amount=${total_amount_cents/100:.2f}")
            ...
            logger.error(f"[Payment Intent] Stripe error: {stripe_err}", exc_info=True)
            return jsonify(error={'message': f'Payment processing error: {str(stripe_err)}'}), 400
        except Exception as e:
            logger.error(f"[Payment Intent] Error creating payment intent: {e}", exc_info=True)
...
def stripe_webhook():
    """
    Handles Stripe webhook events for real-time payment notifications.
    ...
    if event_type == 'payment_intent.succeeded':
        ...
        logger.info(f"[Webhook] ✅ Payment succeeded!")
        logger.info(f"[Webhook] Payment Intent: {payment_intent_id}")
        logger.info(f"[Webhook] Amount: ${amount/100:.2f} {currency.upper()}")
        ...
        try:
            ...
            # Save to orders collection using payment intent ID as document ID
            db.collection('orders').document(payment_intent_id).set(order_data, merge=True)
            logger.info(f"[Webhook] ✅ Order saved to Firestore: orders/{payment_intent_id}")
            ...
            send_sale_notification(
                ...
                currency=currency,
                product_name=metadata.get('product_name'),
                customer_email=event_data.get('receipt_email')
            )

        except Exception as e:
            logger.error(f"[Webhook] Failed to save order to Firestore: {e}")
    ...
    # =========================
        ...
        logger.warning(f"[Webhook] ❌ Payment failed!")
        logger.warning(f"[Webhook] Payment Intent: {payment_intent_id}")
        logger.warning(f"[Webhook] Error: {error_message}")

        # Log failed payment
        try:
            db.collection('failed_payments').document(payment_intent_id).set({
                'stripePaymentId': payment_intent_id,
                'error': error_message,
                'status': 'failed',
                'createdAt': firestore.SERVER_TIMESTAMP
            ...
            logger.error(f"[Webhook] Failed to log failed payment: {e}")
    ...
    # =========================
        ...
        payment_intent_id = event_data['id']
        logger.info(f"[Webhook] Payment canceled: {payment_intent_id}")
    ...
    # =========================
        ...
        payment_intent_id = event_data['id']
        amount = event_data['amount']
        logger.info(f"[Webhook] Payment intent created: {payment_intent_id}, Amount: ${amount/100:.2f}")
    ...
    # =========================
        ...
        logger.info(f"[Webhook] Charge: {charge_id}")
        logger.info(f"[Webhook] Payment Intent: {payment_intent_id}")
        logger.info(f"[Webhook] Amount Refunded: ${amount_refunded/100:.2f}")

        # Update order status
        if payment_intent_id:
            try:
                db.collection('orders').document(payment_intent_id).update({
                    'status': 'refunded',
                    'amount_refunded': amount_refunded,
                    'refundedAt': firestore.SERVER_TIMESTAMP
                })
                logger.info(f"[Webhook] Order status updated to refunded")
            except Exception as e:
                logger.error(f"[Webhook] Failed to update refund status: {e}")
    ...
    # =========================
        ...
        logger.warning(f"[Webhook] ⚠️ DISPUTE CREATED!")
        logger.warning(f"[Webhook] Dispute ID: {dispute_id}")
        logger.warning(f"[Webhook] Charge: {charge_id}")
        logger.warning(f"[Webhook] Amount: ${amount/100:.2f}")
        logger.warning(f"[Webhook] Reason: {reason}")

        # Log dispute for review
        try:
            db.collection('disputes').document(dispute_id).set({
            ...
            logger.warning(f"[Webhook] Dispute logged - REQUIRES ATTENTION!")
        except Exception as e:
            logger.error(f"[Webhook] Failed to log dispute: {e}")
    ...
    # =========================
        ...
        logger.info(f"[Webhook] Refund updated: {refund_id}, Status: {status}")
