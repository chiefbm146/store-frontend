...
export class SmartMessageRenderer {
    ...
    renderScheduleButtonAboveInput(isInBookingFlow) {
        ...
        if (isInBookingFlow) {
            ...
            // ====== NEW: Create Expand World Button (round, centered above Schedule Workshop) ======
            const expandWorldBtn = document.createElement('button');
            expandWorldBtn.id = 'expand-world-btn';
            expandWorldBtn.className = 'expand-world-btn-persistent';
            expandWorldBtn.title = 'Expand Moon Tide World';
            expandWorldBtn.style.width = '60px';
            expandWorldBtn.style.height = '60px';
            expandWorldBtn.style.borderRadius = '50%';
            expandWorldBtn.style.border = 'none';
            expandWorldBtn.style.background = 'linear-gradient(135deg, #4169E1 0%, #1E90FF 100%)';
            expandWorldBtn.style.color = 'white';
            expandWorldBtn.style.fontSize = '26px';
            expandWorldBtn.style.cursor = 'pointer';
            expandWorldBtn.style.transition = 'all 0.3s ease';
            expandWorldBtn.style.boxShadow = '0 4px 15px rgba(30, 144, 255, 0.4), 0 0 20px rgba(30, 144, 255, 0.2)';
            expandWorldBtn.style.display = 'flex';
            expandWorldBtn.style.alignItems = 'center';
            expandWorldBtn.style.justifyContent = 'center';
            expandWorldBtn.style.marginBottom = '12px';
            expandWorldBtn.style.position = 'relative';
            expandWorldBtn.innerHTML = '<i class="fas fa-globe-americas"></i>';

            // Add hover effect for Expand World button
            expandWorldBtn.addEventListener('mouseenter', () => {
                expandWorldBtn.style.transform = 'scale(1.1) rotateY(10deg)';
                expandWorldBtn.style.boxShadow = '0 6px 20px rgba(30, 144, 255, 0.5), 0 0 30px rgba(30, 144, 255, 0.3)';
            });

            expandWorldBtn.addEventListener('mouseleave', () => {
                expandWorldBtn.style.transform = 'scale(1) rotateY(0deg)';
                expandWorldBtn.style.boxShadow = '0 4px 15px rgba(30, 144, 255, 0.4), 0 0 20px rgba(30, 144, 255, 0.2)';
            });

            // Click handler - navigates to the appropriate world HTML
            expandWorldBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('[Expand World Button] Clicked! Navigating to Moon Tide World...');
                this.handleExpandWorldClick();
            });

            // Append Expand World button first (above Schedule Workshop)
            container.appendChild(expandWorldBtn);
            // ====== END: Expand World Button ======
    ...
    /**
     * Handle Expand World button click - Navigate to appropriate world HTML
     * Deterministically chooses world-desk.html or world-mobile.html based on current page
     */
    handleExpandWorldClick() {
        // Determine if we are on desktop.html or mobile.html
        const isDesktop = window.location.pathname.includes('/desktop.html') ||
                         window.location.pathname.includes('desktop.html');

        // Construct the target URL
        const targetUrl = isDesktop ? './world-desk.html' : './world-mobile.html';

        console.log(`[Expand World] Opening ${targetUrl} for ${isDesktop ? 'desktop' : 'mobile'}`);
